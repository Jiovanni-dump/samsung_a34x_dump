...
if [[ -d /system/preload ]]; then
  echo "[PDP] [pdp_pap__c_p_.sh]  size info for each app folders under /s /p :" > /dev/kmsg
  /system/bin/du /system/preload/ -d 1 -b > /dev/kmsg
...
if [[ -d /system/carrier/preload ]]; then
  echo "[PDP] [pdp_pap__c_p_.sh]  size info for each app folders under /s /c /p :" > /dev/kmsg
  /system/bin/du /system/carrier/preload/ -d 1 -b > /dev/kmsg
